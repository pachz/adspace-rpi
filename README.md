...
├── watchdog.sh               # Source of /opt/adspace/watchdog.sh (host bundle)
├── start-display.sh          # Source of /opt/adspace/start-display.sh (host bundle)
├── set-display-mode.sh       # Apply site-preferred HDMI mode if advertised (host bundle)
...
├── hide-cursor/              # Chromium extension: hide pointer (host bundle)
├── pack-host.sh              # Packs host files into adspace-host.tar.gz for embed / QEMU / releases
...
> **Note:** `watchdog.sh`, `start-display.sh`, `set-display-mode.sh`, `indicate.sh`, `device-info.py`, `command-pubkey`, and `hide-cursor/` are packed by `pack-host.sh` into `adspace-host.tar.gz`. `embed.sh` / QEMU copy that tarball onto the boot partition; bootstrap unpacks it into `/opt/adspace`. Edit the standalone file — do not put those files back into `bootstrap.sh`. `kiosk.env` is still generated by bootstrap (`ADSPACE_URL`).
...
├── set-display-mode.sh       # Apply site-preferred HDMI mode via wlr-randr if advertised
├── indicate.sh               # Identify this Pi — blink ACT LED + flash hostname
├── device-info.py            # Always-on localhost:7224 API — version, serial, signed commands
├── command-pubkey            # Fleet Ed25519 public key for POST /api/command
├── hide-cursor/              # Chromium extension — hide pointer (from host bundle)
├── kiosk.env                 # ADSPACE_URL env var
├── display.env               # Optional per-site DISPLAY_MODE — only on LED-wall devices
...
/boot/firmware/adspace-host.tar.gz  # Host bundle baked by embed.sh; bootstrap unpacks it
...
### Set a custom HDMI mode on one device (LED walls)
Most TVs should keep the EDID-preferred mode (usually 1920x1080). Some LED processor chains advertise the canvas aspect as a *non-preferred* mode — Cage then picks 1080p and the content stretches. Oxygen / Kramer is `1920x960` on `HDMI-A-2`.

```bash
# See what the chain actually advertises:
ssh pi@adspace-{serial} "sudo -u adspace env XDG_RUNTIME_DIR=/run/user/\$(id -u adspace) WAYLAND_DISPLAY=wayland-0 wlr-randr"

# Persist the preference on that Pi only (re-applied each time cage starts):
make display-mode PI_SSH=pi@adspace-{serial} MODE=1920x960 OUTPUT=HDMI-A-2

# Undo (back to EDID preferred):
make display-mode PI_SSH=pi@adspace-{serial} CLEAR=1
```

If the requested mode is not in the `wlr-randr` list, the script leaves the compositor alone and logs `mode … not advertised`. Do not bake `DISPLAY_MODE` into the fleet flash images.
...
```
This publishes `wifi-setup-api` (arm64 binary), `wifi-setup-dist.tar.gz`, `adspace-host.tar.gz`, `adspace-tv-v1.2.3.img.xz` (prod), and `adspace-tv-v1.2.3-dev.img.xz` (dev) to GitHub Releases. Newly provisioned Pis pull the latest app artifacts; the host bundle is baked into the flash image. Existing Pis need `make deploy`.
...
The same release also includes `adspace-host.tar.gz` (watchdog, start-display, set-display-mode, indicate, device-info, hide-cursor). That tarball is baked onto the boot partition by `embed.sh`; bootstrap unpacks it and does **not** pull it from GitHub yet.
...
### Site-preferred HDMI mode, not a fleet default
Cage picks the EDID-preferred mode. HDMI/EDID describes the processor, not the LED canvas, so we cannot infer `1920x960` automatically. `/opt/adspace/display.env` is optional and per-device; `set-display-mode.sh` applies that mode with `wlr-randr` only when the chain advertises it.
