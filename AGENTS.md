...
`watchdog.sh`, `start-display.sh`, `set-display-mode.sh`, `indicate.sh`, `device-info.py`, `command-pubkey`, and `hide-cursor/` are the source of truth. `pack-host.sh` puts them in `adspace-host.tar.gz`; `embed.sh` and `qemu-run.sh` copy that onto the boot partition; bootstrap unpacks it into `/opt/adspace`. **Edit the standalone file — do not copy these back into `bootstrap.sh`.** `kiosk.env` is still generated by bootstrap from `ADSPACE_URL`.
...
# Per-device HDMI mode (LED walls). No-op on other Pis — only those with display.env.
...
GitHub Actions creates the git tag and builds the Go API, frontend tarball, host bundle, and both flashable `.img.xz` files (prod + dev).

Manual tag still works (no commit body — release notes are auto-generated only):
```bash
git tag v1.2.3 && git push origin v1.2.3
```
Newly provisioned Pis pull `wifi-setup-api` + `wifi-setup-dist.tar.gz` from the latest release. The host bundle (`adspace-host.tar.gz`) is attached to the same release and baked into the flash image by `embed.sh`. Existing Pis need `make deploy`. Flash new SD cards from `adspace-tv-v1.2.3.img.xz` (prod) or `adspace-tv-v1.2.3-dev.img.xz` (dev) on the same release.
...
### 16. Do not hard-code one HDMI mode for every site
Cage/wlroots picks the EDID-preferred mode (often 1920x1080). Some LED processor chains advertise the canvas aspect as a non-preferred mode (Oxygen: `1920x960` on `HDMI-A-2`). Store the preference in `/opt/adspace/display.env` and let `set-display-mode.sh` apply it only if `wlr-randr` lists that mode. If it is missing, leave the EDID preferred mode alone. HDMI/EDID cannot infer the physical LED pixel map.

### 17. Host scripts live in the repo root, not in bootstrap.sh
`watchdog.sh`, `start-display.sh`, `set-display-mode.sh`, `indicate.sh`, `device-info.py`, `command-pubkey`, and `hide-cursor/` are packed by `pack-host.sh` into `adspace-host.tar.gz`. Bootstrap unpacks that tarball from `/boot/firmware` (step 7). Freshly provisioned Pis get whatever `embed.sh` baked. **Do not put those files back into `bootstrap.sh` as heredocs.** `kiosk.env` is still written by bootstrap.
...
2. Installs all packages: chromium, cage, libwlroots-0.18, caddy, NetworkManager, grim, wlr-randr, jq, python3-cryptography, etc. Dumps Chromium's real UA and writes `/opt/adspace/chromium-ua` as `<ua> AdspaceTV/rpi-<tag>`
...
7. Unpacks `/boot/firmware/adspace-host.tar.gz` into `/opt/adspace/` (`watchdog.sh`, `start-display.sh`, `set-display-mode.sh`, `indicate.sh`, `device-info.py`, `command-pubkey`, `hide-cursor/`) and writes `kiosk.env` (optional `display.env` from `adspace-display.env` on the boot partition)
...
| `/opt/adspace/set-display-mode.sh` | Apply site-preferred HDMI mode via wlr-randr if advertised — no-op without `display.env` |
| `/opt/adspace/display.env` | Optional per-site `DISPLAY_MODE` / `DISPLAY_OUTPUT` — not fleet-wide, not in the host tarball |
...
| `/opt/adspace/hide-cursor/` | Chromium extension that hides the pointer — from the host bundle |
...
| `/boot/firmware/adspace-host.tar.gz` | Host bundle baked by `embed.sh` / `qemu-run.sh`; bootstrap unpacks it into `/opt/adspace` |
...
| `/boot/firmware/adspace-display.env` | Optional `DISPLAY_MODE` / `DISPLAY_OUTPUT` — site-specific LED wall; do not bake into fleet images |
    ...
    "outputs": [
      {
        ...
        "connected": true,
        "enabled": true,
        "make": "Kramer Electronics Ltd. International",
        "model": "SWT3-21-H",
        "current": { "mode": "1920x960", "width": 1920, "height": 960, "refreshHz": 59.987 },
        "preferred": { "mode": "1920x1080", "width": 1920, "height": 1080, "refreshHz": 60.0 },
        "overridden": true,
        "modes": [
          { "mode": "1920x1080", "width": 1920, "height": 1080, "refreshHz": 60.0, "current": false, "preferred": true },
          { "mode": "1920x960", "width": 1920, "height": 960, "refreshHz": 59.987, "current": true, "preferred": false }
        ]
      }
    ]
  },
...
`display` is the HDMI chain as Cage sees it (`wlr-randr`), or DRM sysfs if Wayland is down. `configured` is `/opt/adspace/display.env` (null on TVs that keep the EDID default). `current` is the live mode; `preferred` is the EDID-preferred mode Cage would pick on its own. `applied` is true when the live mode matches `configured`; `overridden` is true when that applied mode is not the EDID preferred. `source` is `wlr-randr`, `drm`, or null.
...
### Set a site-preferred HDMI mode (LED walls)
```bash
# List advertised modes first (Oxygen / Kramer: look for 1920x960):
ssh pi@adspace-{serial} "sudo -u adspace env XDG_RUNTIME_DIR=/run/user/\$(id -u adspace) WAYLAND_DISPLAY=wayland-0 wlr-randr"

make display-mode PI_SSH=pi@adspace-{serial} MODE=1920x960 OUTPUT=HDMI-A-2
# Writes /opt/adspace/display.env and applies now if the chain advertises that mode.
# Re-applied in the background each time cage starts. Other Pis stay on EDID preferred.
...
| Baking one HDMI mode into every image | LED processors advertise HDMI modes, not the canvas; only set `display.env` on the sites that need it |
| Editing host scripts inside bootstrap.sh instead of the standalone files | Dual copies drift; new Pis get whatever was last baked via `pack-host.sh` / `embed.sh` |
| Changing `command-pubkey` without updating `DEFAULT_PUBKEY_HEX` | Fallback verify in `device-info.py` keeps the old fleet key |
